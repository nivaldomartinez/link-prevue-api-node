...
// .wrangler/tmp/bundle-1fBIa9/checked-fetch.js
...
// .wrangler/tmp/bundle-1fBIa9/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();
...
// .wrangler/tmp/bundle-1fBIa9/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
  ...
  constructor(req, options) {
    ...
    this.newResponse = (data2, arg, headers) => {
      ...
      if (arg && typeof arg !== "number") {
        this.res = new Response(data2, arg);
      }
    ...
    this.text = (text3, arg, headers) => {
      ...
      return typeof arg === "number" ? this.newResponse(text3, arg, headers) : this.newResponse(text3, arg);
    };
    ...
    this.html = (html3, arg, headers) => {
      ...
      if (typeof html3 === "object") {
        ...
        if (html3 instanceof Promise) {
          return html3.then((html22) => resolveStream(html22)).then((html22) => {
...
var _Hono = class extends defineDynamicClass() {
  ...
  route(path, app2) {
    ...
    app2.routes.map((r) => {
      const handler = app2.errorHandler === errorHandler ? r.handler : async (c, next2) => (await compose([], app2.errorHandler)(c, () => r.handler(c, next2))).res;
...
var getByProp = ($2, property) => $2(`meta[name="${property}"]`).first().attr("content") || $2(`meta[property="${property}"]`).first().attr("content") || null;
function collectMeta($2, url) {
  const res = {
    url,
    image: getByProp($2, "og:image"),
    imageWidth: getByProp($2, "og:image:width"),
    imageHeight: getByProp($2, "og:image:height"),
    imageType: getByProp($2, "og:image:type"),
    title: getByProp($2, "og:title") || $2("title").text(),
    description: getByProp($2, "og:description"),
    siteName: getByProp($2, "og:site_name") || $2("title").text()
  };
  return Promise.resolve(res);
}
var linkPreview = async (url) => {
  if (!url || url === "")
    return Promise.reject({ message: "You must add a valid url" });
  if (!url.match(/^http(s)?:\/\/[a-z]+\.[a-z]+(.)+/i))
    return Promise.resolve(getEmptyData(url));
  ...
  if (response.status === 200) {
    ...
    return collectMeta(load(body, { decodeEntities: true }), url);
  }
  return getEmptyData(url);
};
var link_preview_default = linkPreview;
...
app.get("/preview", async (c) => {
  ...
  try {
    const preview = await link_preview_default(uri);
...
// .wrangler/tmp/bundle-1fBIa9/middleware-insertion-facade.js
var envWrappers = [wrap2].filter(Boolean);
var facade = {
  ...src_default,
  envWrappers,
  middleware: [
